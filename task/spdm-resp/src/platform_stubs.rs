...
use spdm_lib::cert_store::{SpdmCertStore, CertStoreError};
...
impl SpdmHash for Sha384Hash {
    ...
    fn hash(&mut self, _hash_algo: SpdmHashAlgoType, _data: &[u8], _hash: &mut [u8]) -> SpdmHashResult<()> { Ok(()) }
    fn init(&mut self, _hash_algo: SpdmHashAlgoType, _data: Option<&[u8]>) -> SpdmHashResult<()> { Ok(()) }
    fn update(&mut self, _data: &[u8]) -> SpdmHashResult<()> { Ok(()) }
    fn finalize(&mut self, _out: &mut [u8]) -> SpdmHashResult<()> { Ok(()) }
    fn reset(&mut self) { }
...
impl SpdmCertStore for DemoCertStore {
    ...
    fn cert_chain_len(&mut self, _algo: AsymAlgo, _slot: u8) -> Result<usize, CertStoreError> { Ok(0) }
    fn get_cert_chain(&mut self, _slot: u8, _algo: AsymAlgo, _offset: usize, _out: &mut [u8]) -> Result<usize, CertStoreError> { Ok(0) }
    fn root_cert_hash(&mut self, _slot: u8, _algo: AsymAlgo, _out: &mut [u8; 48]) -> Result<(), CertStoreError> { Ok(()) }
...
impl SpdmEvidence for DemoEvidence {
    ...
    fn pcr_quote(&self, _pcr_index: &mut [u8], _out: bool) -> Result<usize, SpdmEvidenceError> { Ok(0) }
